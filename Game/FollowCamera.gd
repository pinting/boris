...
func _physics_process(dt):
	var target = get_node(target_node).get_global_transform().origin
	...
	params.from = target
	params.to = target + Basis(Vector3.UP, deg_to_rad(autoturn_ray_aperture)) * delta
	params.exclude = collision_exception

	var col_left = ds.intersect_ray(params)

	params = PhysicsRayQueryParameters3D.new()

	params.from = target
	params.to = target + delta
	params.exclude = collision_exception
	
	var col = ds.intersect_ray(params)
	...
	var col_right = ds.intersect_ray(params)
